...
export class OlimpiadasApiService {
  ...
  static async createOlimpiada(data: OlimpiadaFormData): Promise<Olimpiada> {
    try {
      ...
      const response = await api.post('/api/v1/tenant/tests', payload);
  ...
  static async getOlimpiadas(params?: {
    ...
    try {
      ...
      let response;
      try {
        response = await api.get(`/api/v1/tenant/tests?${queryParams.toString()}`);
      } catch (error) {
        ...
        response = await api.get(`/api/v1/tenant/tests?${allParams.toString()}`);
      }
  ...
  static async getOlimpiada(id: string): Promise<Olimpiada> {
    try {
      const response = await api.get(`/api/v1/tenant/tests/${id}`);
      const result = response.data;
  ...
  static async getAppliedClasses(olimpiadaId: string): Promise<AppliedClass[]> {
    try {
      const response = await api.get(`/api/v1/tenant/tests/${olimpiadaId}/classes`);
  ...
  static async removeAppliedClass(testId: string, classId: string): Promise<void> {
    await api.delete(`/api/v1/tenant/tests/${testId}/classes/${classId}`);
  }
  ...
  static async removeAppliedClasses(testId: string, classIds: string[]): Promise<void> {
    await api.post(`/api/v1/tenant/tests/${testId}/classes/remove`, { class_ids: classIds });
  }
  ...
  static async updateOlimpiada(id: string, data: Partial<OlimpiadaFormData>): Promise<Olimpiada> {
    try {
      ...
      const response = await api.put(`/api/v1/tenant/tests/${id}`, payload);
  ...
  static async deleteOlimpiada(id: string): Promise<void> {
    try {
      await api.delete(`/api/v1/tenant/tests/${id}`);
    } catch (error) {
  ...
  static async applyOlimpiadaToStudents(
    ...
    try {
      ...
      console.log('📡 [olimpiadasApi] Aplicando olimpíada para alunos individuais:', {
        endpoint: `/api/v1/tenant/tests/${id}/apply-olympics`,
      ...
      const results = await Promise.all(
        studentIds.map(async (studentId) => {
          const response = await api.post(`/api/v1/tenant/tests/${id}/apply-olympics`, {
  ...
  static async applyOlimpiada(
    ...
    try {
      ...
      console.log('📡 [olimpiadasApi] Enviando dados para backend:', {
        endpoint: `/api/v1/tenant/tests/${id}/apply`,
        payload: {
      ...
      const response = await api.post(`/api/v1/tenant/tests/${id}/apply`, {
        classes: classesPayload,
        timezone: userTimezone
      });
  ...
  static async getIndividualAppliedStudents(olimpiadaId: string): Promise<string[]> {
    try {
      try {
        ...
        const response = await api.get(`/api/v1/tenant/tests/${olimpiadaId}/applied-students`, {
  ...
  static async removeIndividualApplication(olimpiadaId: string, studentId: string): Promise<void> {
    await api.delete(`/api/v1/tenant/tests/${olimpiadaId}/olympics/${studentId}`);
  }
  ...
  static async getStudentOlimpiadas(): Promise<Olimpiada[]> {
    try {
      const response = await api.get('/api/v1/tenant/tests/my-class/tests');
