    ...
    switch (role) {
        ...
        try {
          ...
          if (userData.city_id) {
            try {
              ...
              const schoolsResponse = await api.get(`/v1/tenant/schools`);
              ...
              if (userSchoolId) {
                try {
                  const schoolResponse = await api.get(`/v1/tenant/schools/${userSchoolId}`);
              ...
              if (managerSchoolId) {
                try {
                  const schoolResponse = await api.get(`/v1/tenant/schools/${managerSchoolId}`);
                  const schoolData = schoolResponse.data;
      ...
      case 'professor':
        ...
        try {
          ...
          try {
            const byUserResponse = await api.get(`/api/v1/tenant/teachers/${userId}`);
          ...
          if (!teacherData?.turmas?.length && !teacherData?.classes?.length) {
            const listResponse = await api.get('/api/v1/tenant/teachers');
            ...
            if (teacher?.id) {
              ...
              try {
                const byIdResponse = await api.get(`/api/v1/tenant/teachers/${teacher.id}`);
          ...
          if (turmas && turmas.length > 0) {
            ...
            let municipality: { id: string; name: string; state: string } | undefined;
            try {
              const schoolResponse = await api.get(`/v1/tenant/schools/${schoolId}`);
              const schoolData = schoolResponse.data;
              if (schoolData?.city_id) {
                const municipalityResponse = await api.get(`/city/${schoolData.city_id}`);
                const municipalityData = municipalityResponse.data;
          ...
          if (teacherId) {
            try {
              const linksResp = await api.get('/api/v1/tenant/teachers', { params: { teacher_id: teacherId } });
