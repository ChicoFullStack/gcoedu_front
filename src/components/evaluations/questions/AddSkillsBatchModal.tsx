...
const AddSkillsBatchModal = ({
  ...
  useEffect(() => {
    ...
    setError(null);
    setBatchErrors([]);
  }, [open, defaultSubjectId, defaultGradeId]);
  ...
  const addRow = () => {
    if (rows.length >= 200) {
      setError("Adicione no máximo 200 habilidades por vez.");
      return;
    }
    setRows((prev) => [
      ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validationErrors = skills.flatMap((skill, index) => {
      if (!skill.code) return [{ index, error: "Código obrigatório" }];
      if (!skill.description) return [{ index, error: "Descrição obrigatória" }];
      return [];
    });
    const duplicateCodes = new Set<string>();
    const seenCodes = new Set<string>();
    skills.forEach((skill) => {
      const key = skill.code.toLowerCase();
      if (seenCodes.has(key)) duplicateCodes.add(key);
      seenCodes.add(key);
    });
    skills.forEach((skill, index) => {
      if (duplicateCodes.has(skill.code.toLowerCase())) {
        validationErrors.push({ index, error: `Código repetido no lote: ${skill.code}` });
      }
    });
    if (validationErrors.length > 0) {
      setBatchErrors(validationErrors);
      setError("Corrija as linhas indicadas antes de enviar.");
      return;
    }

    setLoading(true);
    try {
      const payload = skills.map(({ code, description, subject_id, grade_id }) => {
      ...
      if (created > 0) {
        ...
        if (errs.length === 0) {
          ...
          const failedCodes = new Set(
            errs
              .map((item) => payload[item.index]?.code.toLowerCase())
              .filter((code): code is string => Boolean(code))
          );
          setRows((currentRows) =>
            currentRows.filter((row) => failedCodes.has(row.code.trim().toLowerCase()))
          );
