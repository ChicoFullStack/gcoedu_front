...
interface AvailableUserApiRecord {
  id?: unknown;
  name?: unknown;
  email?: unknown;
  registration?: unknown;
  role?: unknown;
  user?: AvailableUserApiRecord;
}

interface AvailableUsersApiResponse {
  users?: AvailableUserApiRecord[];
  managers?: AvailableUserApiRecord[];
}

function normalizeAvailableUser(record: AvailableUserApiRecord): User | null {
  const source = record.user ?? record;
  const id = source.id == null ? "" : String(source.id);
  const role = source.role == null ? "" : String(source.role).toLowerCase();
...
export function LinkDirectorCoordinatorModal({
  ...
  const fetchUsers = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      // A listagem legada /teacher/directors|coordinators falha no backend local.
      // Priorizar usuários do município; sem cityId, usar a listagem consolidada de gestores.
      const endpoint = schoolCityId
        ? `/city/${schoolCityId}/users`
        : '/users/managers';
      const response = await api.get(endpoint);

      const data = response.data as AvailableUsersApiResponse | AvailableUserApiRecord[];
      ...
      const allUsers = rawUsers
        ...
        .filter((availableUser): availableUser is User =>
          availableUser !== null && availableUser.role === userType
        );
  ...
  }, [isOpen, schoolCityId, userType, toast]);
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      if (formData.registration) {
        userData.registration = formData.registration;
      }
