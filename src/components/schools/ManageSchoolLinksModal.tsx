...
export function ManageSchoolLinksModal({
  ...
  const fetchLinks = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await api.get<SchoolLink[] | ManagersApiResponse>(
        `/managers/city/${schoolCityId}`
      );
      ...
      setLinks(
        managerLinks.filter(
          (link) =>
            link?.user?.id &&
            link?.manager?.school_id != null &&
            String(link.manager.school_id) === schoolId
        )
      );
    } catch (error) {
      ...
      toast({
        title: "Erro",
        description: "Erro ao carregar vínculos da escola",
        variant: "destructive",
      });
      setLinks([]);
    } finally {
