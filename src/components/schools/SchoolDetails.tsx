...
import { UserPlus, Eye, Pencil, Trash2, Edit, Loader2, ArrowLeft, Building, Users, GraduationCap, MapPin, Globe, Calendar, Plus, BookOpen, School, Upload, MoveRight, Link2, Clock } from "lucide-react";
...
interface ManagerLinkPayload {
  school_id?: unknown;
  school?: { id?: unknown };
  user?: ManagerUserPayload;
  manager?: ManagerUserPayload;
}

interface ManagersApiResponse {
  managers?: ManagerLinkPayload[];
}

function normalizeManagerUser(managerLink: ManagerLinkPayload): Teacher | null {
  const managerUser = managerLink.user ?? managerLink.manager;
  if (!managerUser?.id) return null;

  return {
    id: String(managerUser.id),
    name: managerUser.name == null ? "" : String(managerUser.name),
    email: managerUser.email == null ? "" : String(managerUser.email),
    registration:
      managerUser.registration == null ? undefined : String(managerUser.registration),
    role: managerUser.role == null ? undefined : String(managerUser.role).toLowerCase(),
  };
}
...
export default function SchoolDetails() {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      ...
      try {
        const classesData = await schoolClassApi.findAll(id);
        const gradesResponse = await api
          .get<Array<{ id: string; name: string }>>("/grades/")
          .catch(() => ({ data: [] as Array<{ id: string; name: string }> }));
        const gradeNamesById = new Map(
          gradesResponse.data.map((grade) => [String(grade.id), grade.name])
        );
        const normalizedClasses: Class[] = classesData.map((schoolClass) => ({
          ...schoolClass,
          grade: schoolClass.gradeId
            ? {
                id: schoolClass.gradeId,
                name: gradeNamesById.get(schoolClass.gradeId) ?? schoolClass.gradeId,
        ...
        if (normalizedClasses.length > 0) {
          await fetchClassDetails(normalizedClasses);
        }
      } catch (error) {
        setClasses([]);
      } finally {
  ...
  const loadSchoolTeachers = useCallback(async () => {
    ...
    try {
      const teachersData = await teacherApi.findAll(id);
  ...
  useEffect(() => {
    const fetchSchoolManagers = async () => {
      if (!id || !school?.city_id) return;

      try {
        const response = await api.get<ManagersApiResponse | ManagerLinkPayload[]>(
          `/managers/city/${school.city_id}`
        );
        const managerLinks = Array.isArray(response.data)
          ? response.data
          : response.data?.managers ?? [];
        const schoolManagers = managerLinks
          .filter((managerLink) => {
            const linkedSchoolId =
              managerLink.school_id ??
              managerLink.manager?.school_id ??
              managerLink.school?.id;
            return linkedSchoolId != null && String(linkedSchoolId) === id;
          })
          .map(normalizeManagerUser)
          .filter((manager): manager is Teacher => manager !== null);

        setDirectors(schoolManagers.filter((manager) => manager.role === "diretor"));
        setCoordinators(schoolManagers.filter((manager) => manager.role === "coordenador"));
      } catch {
        setDirectors([]);
        setCoordinators([]);
      }
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      ...
      try {
        const alunos = await studentApi.findAll(id);
        setStudents(alunos);
      } catch (error) {
        toast({
          title: "Erro",
