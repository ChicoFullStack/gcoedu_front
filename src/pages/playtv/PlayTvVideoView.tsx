...
export default function PlayTvVideoView() {
  ...
  const fetchVideo = useCallback(async () => {
    ...
    try {
      ...
      if (error.response?.status === 404) {
        setError('Vídeo não encontrado ou indisponível para o seu perfil');
      } else {
  ...
  const canEdit =
    ...
    contextLoaded &&
    video !== null &&
    canEditPlayTvVideo(video, user.role, { ...userContext, userId: user.id });
