...
export default function Login() {
  ...
  const handleLogin = async (e?: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const emailCompleto = `${matriculaLimpa}@gcoedu.com.br`;
  ...
  return (
    <div 
      ...
      className={`min-h-screen w-full flex flex-col lg:flex-row transition-all duration-500 fixed inset-0 z-50 bg-[#0C4A6E]`}
    >
      {/* Lado esquerdo (gradiente GCOEDU) */}
      <div className="text-white w-full lg:w-1/2 flex flex-col justify-center items-center p-6 lg:p-12 relative overflow-hidden transition-all duration-500 bg-[#0C4A6E]">
        ...
        <div className="max-w-md flex flex-col items-center text-center relative z-10">
          ...
          <div className={`mb-8 w-32 h-32 md:w-full md:h-full flex items-center justify-center flex-col transition-opacity duration-300 ${
            ...
            <div className={`mt-6 w-24 h-1 rounded-full transition-opacity duration-300 bg-sky-400/50 ${
      ...
      <div className="flex flex-col justify-center items-center p-6 lg:p-12 w-full lg:w-1/2">
        <div className="w-full max-w-md">
          <div className={`relative flex flex-col transition-all duration-300 bg-gradient-to-br from-[#0C4A6E] to-[#075985] shadow-[7px_7px_10px_3px_rgba(36,0,70,0.16)] border border-blue-500/20 rounded-2xl overflow-hidden hover:shadow-2xl hover:scale-[1.01] ${
            isMounted ? "animate-slide-in-right opacity-100" : "opacity-0 translate-x-8"
          }`}>
            {/* Gradiente sutil no topo */}
            <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 via-sky-500 to-emerald-500"></div>
            ...
            <div className="flex flex-col gap-5 p-8">
              ...
              {/* Divisor */}
              <div className="h-px w-full my-2 bg-gradient-to-r from-transparent via-blue-500/30 to-transparent"></div>
              ...
              >
                <div className="space-y-1">
                  ...
                  <div className={`relative group transition-opacity duration-300 delay-75 ${
                    ...
                    <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none transition-transform duration-300 group-focus-within:scale-110 z-10">
                      <User className="h-5 w-5 transition-colors duration-300 text-gray-400 group-focus-within:text-sky-400" />
                    </div>
                    <div className="relative flex items-center">
                      <Input
                        type="text"
                        placeholder="usuario"
                        className="pl-12 pr-32 h-12 bg-gray-700/80 text-white placeholder:text-gray-500 border-none outline-none rounded-lg transition-all duration-300 focus:scale-[1.01] focus:shadow-lg focus:bg-gray-700 focus:ring-2 focus:ring-blue-500/50"
                        value={matricula}
                        onChange={(e) => {
                ...
                <div className="space-y-1">
                  ...
                  <div className={`relative group transition-opacity duration-300 delay-150 ${
                    ...
                    <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none transition-transform duration-300 group-focus-within:scale-110">
                      <Lock className="h-5 w-5 transition-colors duration-300 text-gray-400 group-focus-within:text-sky-400" />
                    </div>
                    <Input
                      type={showPassword ? "text" : "password"}
                      placeholder="Digite sua senha"
                      className="pl-12 pr-12 h-12 bg-gray-700/80 text-white placeholder:text-gray-500 border-none outline-none rounded-lg transition-all duration-300 focus:scale-[1.01] focus:shadow-lg focus:bg-gray-700 focus:ring-2 focus:ring-blue-500/50"
                      value={senha}
                    ...
                    >
                      {showPassword ? (
                        <EyeOff className="h-5 w-5 transition-all duration-300 text-gray-400 hover:text-sky-400" />
                      ) : (
                        <Eye className="h-5 w-5 transition-all duration-300 text-gray-400 hover:text-sky-400" />
                      )}
                    </button>
                  </div>
                </div>
                ...
                <div className={`flex items-center justify-between pt-2 transition-opacity duration-300 ${
                  ...
                  <div className="flex items-center space-x-2.5">
                    ...
                    <label
                      htmlFor="lembrar"
                      className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 transition-colors duration-300 cursor-pointer text-white/90 hover:text-sky-300"
                    >
                      Lembrar-me
                    </label>
                  </div>
                  <a 
                    ...
                    className="text-sm font-medium transition-all duration-300 hover:underline hover:scale-105 text-sky-300 hover:text-sky-200"
                  >
                    Esqueceu a senha?
                  </a>
                </div>
          ...
          <div className={`mt-6 text-center text-xs transition-opacity duration-300 text-white/60 ${
            ...
            © {new Date().getFullYear()} GCOEDU - JESUS CRISTO É O SENHOR
          </div>
        </div>
      </div>
    </div>
