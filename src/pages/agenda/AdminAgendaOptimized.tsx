...
import { parseISOToDatetimeLocal, toLocalOffsetISO } from "@/utils/date";
...
export default function AdminAgendaOptimized() {
  ...
  const isMutationLoading = isCreatingEvent || isUpdatingEvent || isDeletingEvent;
  ...
  const validateEventForm = (): boolean => {
    ...
    if (title.length > 160) {
      toast.error('O título deve ter no máximo 160 caracteres');
      return false;
    }
    ...
    if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) {
      toast.error('Informe datas válidas para o evento');
      return false;
    }
    ...
    if (formData.location.trim().length > 255) {
      toast.error('O local deve ter no máximo 255 caracteres');
      return false;
    }

    const links = isEditOpen ? editLinkResources : createLinkResources;
    for (const resource of links) {
      const linkTitle = resource.title.trim();
      const linkUrl = resource.url.trim();
      if (!linkTitle && !linkUrl) continue;
      if (!linkTitle || !linkUrl) {
        toast.error('Preencha o título e a URL de cada link, ou remova a linha vazia');
        return false;
      }
      try {
        ...
        toast.error('Informe uma URL válida iniciada por http:// ou https://');
        return false;
      }
    }

    const pendingFiles = isEditOpen ? pendingEditFiles : pendingCreateFiles;
    if (pendingFiles.some((file) => file.size > 10 * 1024 * 1024)) {
      toast.error('Cada anexo deve ter no máximo 10 MB');
      return false;
    }
  ...
  const handleDateSelect = (selectInfo: DateSelectArg) => {
    ...
    setFormData((f) => ({
      ...
      description: '',
      location: '',
      startTime: parseISOToDatetimeLocal(start.toISOString()),
      endTime: parseISOToDatetimeLocal(end.toISOString()),
  ...
  const openEditFromSelected = () => {
    ...
    setFormData({
      ...
      startTime: parseISOToDatetimeLocal(selected.start as string),
      endTime: parseISOToDatetimeLocal(selected.end as string),
  ...
  return (
    <div className="p-4">
      ...
      <Dialog open={isCreateOpen} onOpenChange={(open) => {
        ...
        >
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="title" required maxLength={160} value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} placeholder="Título do evento" />
              </div>
            </div>
            <div className="space-y-2">
              <Label htmlFor="description">Descrição</Label>
              <Textarea id="description" maxLength={5000} value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Descrição do evento" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="start">Data/Hora Início</Label>
                <Input id="start" required type="datetime-local" value={formData.startTime} onChange={(e) => setFormData({ ...formData, startTime: e.target.value })} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="end">Data/Hora Fim</Label>
                <Input id="end" required min={formData.startTime || undefined} type="datetime-local" value={formData.endTime} onChange={(e) => setFormData({ ...formData, endTime: e.target.value })} />
              </div>
            </div>
            <div className="space-y-2">
              <Label htmlFor="location">Local</Label>
              <Input id="location" maxLength={255} value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} placeholder="Local do evento" />
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={createEvent} disabled={isCreatingEvent || isLoadingTargets}>
      ...
      <Dialog
        open={isEditOpen}
        onOpenChange={(open) => {
          if (!isUpdatingEvent) setIsEditOpen(open);
        }}
      >
        <DialogContent className="max-w-3xl max-h-[95vh] sm:max-h-[90vh] overflow-y-auto w-[95vw] sm:w-full">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="edit-title" required maxLength={160} value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} />
              </div>
            </div>
            <div className="space-y-2">
              <Label htmlFor="edit-description">Descrição</Label>
              <Textarea id="edit-description" maxLength={5000} value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="edit-start">Início</Label>
                <Input id="edit-start" required type="datetime-local" value={formData.startTime} onChange={(e) => setFormData({ ...formData, startTime: e.target.value })} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="edit-end">Fim</Label>
                <Input id="edit-end" required min={formData.startTime || undefined} type="datetime-local" value={formData.endTime} onChange={(e) => setFormData({ ...formData, endTime: e.target.value })} />
              </div>
            </div>
            <div className="space-y-2">
              <Label htmlFor="edit-location">Local</Label>
              <Input id="edit-location" maxLength={255} value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} />
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsEditOpen(false)} disabled={isUpdatingEvent}>Cancelar</Button>
            <Button onClick={updateEvent} disabled={isUpdatingEvent || isLoadingTargets}>
              {isUpdatingEvent ? 'Salvando...' : 'Salvar'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
