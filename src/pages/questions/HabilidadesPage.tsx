...
import { AlertCircle, BookOpen, Search, Target, List, Grid, ArrowLeft, Filter, Plus, RefreshCw, Trash2 } from "lucide-react";
...
const mapSkill = (skill: SkillResponse): Skill => ({
  id: skill.id,
  code: skill.code,
...
const HabilidadesPage = () => {
  ...
  const canManageSkills = ALLOWED_SKILL_ROLES.includes(role);
  const canAddBatch = BATCH_ADD_ROLES.includes(role);
  const canDeleteSkills = DELETE_SKILL_ROLES.includes(role);
  ...
  const loadReferenceData = useCallback(async () => {
    ...
    try {
      ...
      console.error("Erro ao carregar disciplinas e séries:", error);
      setReferenceError("Não foi possível carregar as disciplinas e séries.");
    }
  }, []);
  ...
  const loadSkills = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.get<SkillResponse[]>("/skills", { params });
      setSkills(Array.isArray(response.data) ? response.data.map(mapSkill) : []);
      setSelectedSkillIds([]);
    } catch (error) {
      console.error("Erro ao carregar habilidades:", error);
      setSkills([]);
      setLoadError("Não foi possível carregar o banco de habilidades.");
    } finally {
      setLoading(false);
    }
  }, [subjectId, gradeId]);
  ...
  return (
    <div className="container mx-auto py-4 px-2 sm:px-4 space-y-4">
      ...
      <div className="bg-muted/30 p-3 sm:p-4 rounded-lg space-y-3">
        {referenceError && (
          <div className="flex flex-col gap-2 rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive sm:flex-row sm:items-center sm:justify-between">
            <span className="flex items-center gap-2">
              <AlertCircle className="h-4 w-4 shrink-0" />
              {referenceError}
            </span>
            <Button type="button" variant="outline" size="sm" onClick={loadReferenceData}>
          ...
          <ScrollArea className="flex-1 min-h-[400px]">
            <div className="p-3 sm:p-4 space-y-6">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  <AlertCircle className="mb-4 h-12 w-12 text-destructive opacity-70" />
                  <p className="mb-2 text-lg font-medium text-foreground">Erro ao carregar habilidades</p>
                  <p className="mb-4 text-sm text-muted-foreground">{loadError}</p>
                  <Button type="button" variant="outline" onClick={loadSkills}>
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {skills.length === 0
                      ? subjectId === "all" && gradeId === "all"
                        ? "Nenhuma habilidade cadastrada no catálogo BNCC."
                        : "Nenhuma habilidade cadastrada para os filtros selecionados."
