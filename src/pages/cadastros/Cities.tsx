...
/** Retorna URL e host do subdomínio conforme o ambiente (localhost vs gcoedu.com.br) */
function getSubdomainConfig() {
  if (typeof window === "undefined") {
    return {
      url: (slug: string) => (slug ? `https://${slug}.gcoedu.com.br` : "#"),
      displayHost: "gcoedu.com.br",
    };
  }
  const hostname = window.location.hostname;
  const isLocal = hostname === "localhost" || hostname === "127.0.0.1";
  const port = window.location.port || "8080";
  const host = isLocal ? `localhost:${port}` : "gcoedu.com.br";
    ...
    <div className="min-h-screen p-4 sm:p-6 lg:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <Card className="border shadow-sm">
          <CardContent className="pt-6 space-y-4">
            ...
            {isLoading ? (
              <div className="flex min-h-[280px] items-center justify-center rounded-xl border border-dashed border-[#BFDBFE] dark:border-white/10 bg-muted/30">
                <Loader2 className="h-10 w-10 animate-spin text-[#2563EB]" />
              </div>
            ) : filteredCities.length === 0 ? (
              <div className="flex min-h-[200px] flex-col items-center justify-center rounded-xl border border-dashed border-[#BFDBFE] dark:border-white/10 bg-muted/20 px-4 text-center">
                ...
                {filteredCities.map((city) => (
                  <Card
                    ...
                    className={cn(
                      ...
                      "border-[#BFDBFE] hover:border-[#2563EB]/40 hover:shadow-md dark:border-white/10 dark:hover:border-[#2563EB]/40"
                    )}
                  >
                      ...
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="space-y-2">
                          <a
                            ...
                            className={cn(
                              "inline-flex items-center gap-1.5 rounded-lg border border-[#BFDBFE] dark:border-white/10",
                              "bg-muted/30 px-3 py-2 text-sm font-mono text-[#2563EB] w-full",
                              "transition-colors hover:bg-[#2563EB]/10 hover:border-[#2563EB]/30"
                            )}
                          >
                        ...
                        <Button
                          ...
                          className="mt-1 w-full gap-2 border-[#BFDBFE] dark:border-white/10"
                        >
      ...
      <Dialog open={usersModalOpen} onOpenChange={setUsersModalOpen}>
        <DialogContent className="max-h-[90vh] w-[95vw] max-w-2xl overflow-hidden flex flex-col p-0 sm:rounded-lg">
          <DialogHeader className="shrink-0 border-b border-[#BFDBFE] px-4 py-4 dark:border-white/10 sm:px-6">
                  ...
                  return (
                    <div className="space-y-5">
                      {orderedRoles.map((role) => {
                        ...
                        return (
                          <Card key={role} className="overflow-hidden border-[#BFDBFE] dark:border-white/10">
                            <div className="bg-[#2563EB]/5 px-3 py-2 dark:bg-[#2563EB]/10">
                              <h4 className="text-sm font-semibold text-[#2563EB]">
                                {getRoleDisplayName(role)}
                              </h4>
                            </div>
                            <CardContent className="p-0">
                              <ul className="divide-y divide-[#BFDBFE] dark:divide-white/10">
                                {list.map((u) => (
                                  <li
                                    key={u.id}
