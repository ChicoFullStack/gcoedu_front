...
import { schoolClassApi, type SchoolClass } from "@/services/tenant/schoolClassApi";
...
export default function Gestao() {
  ...
  const fetchInstituicoes = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await schoolApi.findAll().then(data => ({ data }));
      ...
      for (let i = 0; i < list.length; i += chunkSize) {
        ...
        const partial = await Promise.all(
          slice.map(async (inst) => {
            try {
              const res = await studentApi.findAll(inst.id);
              return [inst.id, res.length] as const;
            } catch {
  ...
  const fetchInstituicaoDetails = useCallback(async (schoolId: string) => {
    ...
    try {
      // Buscar turmas
      const classesResponse = await schoolClassApi.findAll(schoolId).then(data => ({ data }));
      setClasses(classesResponse.data || []);

      // Buscar professores
      const teachersResponse = await teacherApi.findAll(schoolId).then(data => ({ data }));
      setTeachers(Array.isArray(teachersResponse.data) ? teachersResponse.data : []);

      // Buscar alunos
      const studentsResponse = await studentApi.findAll(schoolId).then(data => ({ data }));
  ...
  const handleSaveInstituicao = async (instituicao: Partial<Instituicao>) => {
    ...
    try {
      ...
      if (selectedInstituicao) {
        ...
        await schoolApi.create(normalizeSchoolWritePayload(instituicao));
  ...
  const handleDeleteInstituicao = async (instituicaoId: string) => {
    setIsDeleting(true);
    try {
      await schoolApi.delete(instituicaoId);
      await fetchInstituicoes(); // Recarrega a lista completa
  ...
  const fetchContactsByRoleForSchool = useCallback(
    async (cityId: string, schoolId: string): Promise<ContactsByRole> => {
      ...
      const [usersRes, managersRes, studentsRes, teachersRes] = await Promise.all([
        ...
        studentApi.findAll(schoolId).then(data => ({ data })).catch(() => ({ data: [] })),
        teacherApi.findAll(schoolId).then(data => ({ data })).catch(() => ({ data: [] })),
      ]);
