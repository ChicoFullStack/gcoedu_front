...
export default function Turmas({ embedded = false }: TurmasProps) {
  ...
  const fetchTurmas = useCallback(async () => {
    try {
      ...
      const schoolsResponse = await schoolApi.findAll().then(data => ({ data }));
  ...
  const updateClassStudentCount = async (classId: string, schoolId: string) => {
    try {
      ...
      const response = await schoolClassApi.findAll(schoolId).then(data => ({ data }));
  ...
  const generateEmail = (fullName: string) => {
    ...
    return `${initials}@gcoedu.com.br`;
  };

  const generatePassword = (fullName: string) => {
    const firstName = fullName.split(" ")[0].toLowerCase();
    return `${firstName}@gcoedu`;
  };
                    ...
                    <CardHeader>
                      ...
                      <div className="text-sm text-muted-foreground bg-blue-50 p-3 rounded-lg border border-blue-200">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                          <div>
                            <p><strong>Email:</strong> Iniciais do nome + "@gcoedu.com.br"</p>
                            <p className="text-blue-600 font-mono">Ex: "João Silva" → jss@gcoedu.com.br</p>
                          </div>
                          <div>
                            <p><strong>Senha:</strong> Primeiro nome + "@gcoedu"</p>
                            <p className="text-blue-600 font-mono">Ex: "João Silva" → joão@gcoedu</p>
                          </div>
                        </div>
