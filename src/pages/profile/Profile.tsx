...
const Profile = () => {
  ...
  // Usar dados do perfil quando disponível
  const studentData = user?.role === 'aluno' && profileData ? (profileData as StudentData) : null;
  const userData = user?.role !== 'aluno' && user?.role !== 'professor' && profileData ? (profileData as UserData) : null;
  
  const personalDetails = {
    "Nome completo": studentData?.full_name || studentData?.name || userData?.name || user?.name || "Nome não informado",
    "Data de Nascimento": studentData?.birth_date ? formatDate(studentData.birth_date) : (userData?.birth_date ? formatDate(userData.birth_date) : (user?.birth_date ? formatDate(user.birth_date) : "Data não informada")),
    "Gênero": userData?.gender || user?.gender || "Gênero não informado",
    "Nacionalidade": userData?.nationality || user?.nationality || "Nacionalidade não informada",
    "Endereço": studentData?.address || userData?.address || user?.address || "Endereço não informado",
    "Telefone": userData?.phone || user?.phone || "Telefone não informado",
    "Email": studentData?.email || userData?.email || user?.email || "usuario@exemplo.com",
  };
